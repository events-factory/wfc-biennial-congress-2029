...
export const abstractsApi = {
  ...
  // Streams the backend-rendered xlsx through the proxy and triggers a browser
  // download. The spreadsheet is built server-side, so the client only forwards
  // the auth token and saves the returned file.
  exportExcel: async (): Promise<void> => {
    ...
    const response = await fetch(`${API_BASE_URL}/abstracts/export`, {
      method: 'GET',
      headers,
    });
    if (!response.ok) {
    ...
    const filename = match?.[1] || 'abstracts-export.xlsx';
